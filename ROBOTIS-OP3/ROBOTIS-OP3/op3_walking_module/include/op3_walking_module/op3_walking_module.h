...
class WalkingModule : public robotis_framework::MotionModule, public robotis_framework::Singleton<WalkingModule>
{
 ...
 private:
  ...
  void standupCallback(const std_msgs::Int32::ConstPtr &msg);
  
  /* ROS Service Callback Functions */
  void processPhase(const double &time_unit);
  bool computeLegAngle(double *leg_angle);
  void computeArmAngle(double *arm_angle);
  void sensoryFeedback(const double &rlGyroErr, const double &fbGyroErr, const double &filtered_angle, double *balance_angle, const double &target_angle, double &integral_Err);
  ...
  ros::Publisher fb_gyro_angle_pub_;
  ros::Publisher accel_angle_pub_;
  ros::Publisher filtered_angle_pub_;

  ros::Publisher sensor_gyro_x_pub_;
  ros::Publisher sensor_gyro_y_pub_;
  ros::Publisher sensor_gyro_z_pub_;
  ros::Publisher sensor_yaw_pub_;
  ...
  double accel_angle;
  double rl_gyro_angle;
  double fb_gyro_angle;
  double integral_Err;
  double pre_angle;
  double rl_gyro_angle_dt;
  int step_num;
};
