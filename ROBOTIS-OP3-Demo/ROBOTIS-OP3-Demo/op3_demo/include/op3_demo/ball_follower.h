...
class BallFollower
{
 public:
  ...
  void case_move(const std::string &command); // HS
  void case_turn(const std::string &command); // HS
  void case_nm(const std::string &command); // HS
  void waitFollowing();
  void startFollowing();
  void stopFollowing();
 ...
 protected:
  ...
  void gyro_kudos_Callback(const std_msgs::Float64::ConstPtr &msg); //kudos
 ...
 // void process(std::map<std::string, robotis_framework::Dynamixel *> dxls, std::map<std::string, double> sensors);
  ...
  ros::Subscriber sensor_gyro_z_sub_; //kudos
