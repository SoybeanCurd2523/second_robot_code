...
BallFollower::BallFollower()
  : nh_(ros::this_node::getName()),
    ...
    MAX_FB_STEP(20.0 * 0.001), //15
    MAX_RL_TURN(10.0 * M_PI / 180),
    IN_PLACE_FB_STEP(-3.0 * 0.001),
    MIN_FB_STEP(0.0 * 0.001), //5
    ...
    DEBUG_PRINT(false)
{
  ...
  sensor_gyro_z_sub_ = nh_.subscribe("/kubot/sensor_gyro_z", 1000, &BallFollower::gyro_kudos_Callback,this);
...
void BallFollower::startFollowing()
{
  ...
  {
    hip_pitch_offset_ = 15.0 * M_PI / 180;
    curr_period_time_ = 0.6;
  }
}
...
void BallFollower::case_move(const std::string &command) // HS
{
  if (command == "start")
  {
    //curr_period_time_ = current_walking_param_.period_time;
    setWalkingParam(0, 0, -0.01, true);
    setWalkingCommand("start");
  }
}
void BallFollower::case_nm(const std::string &command) // HS
{
  if (command == "start")
  {
    //curr_period_time_ = current_walking_param_.period_time;
    setWalkingParam(0, 0, 0, true);
    setWalkingCommand("start");
  }
}
void BallFollower::case_turn(const std::string &command) // HS
{
  if (command == "start")
  {
    //curr_period_time_ = current_walking_param_.period_time;
    setWalkingParam(0, 0, -0.11, true);
    setWalkingCommand("start");
  }
}

void BallFollower::gyro_kudos_Callback(const std_msgs::Float64::ConstPtr &msg)
{

  float gyro_z = msg->data;
...
//  ROS_INFO("d");
  ...
  {
    accum_period_time_ = 0.0;
    if ((target_distance * 0.05 / 2) < current_x_move_) 
...
//  int ball_position_sum = 0;
  ...
  //double distance_to_kick = 0.25; original code
  double distance_to_kick = 0.22; //0.22  pjh
  ...
  if ((distance_to_ball < distance_to_kick) && (fabs(ball_x_angle) < 25.0))
  {
    ...
    ROS_INFO("ball_x_angle : %f , count_to_kick_ : %d", ball_x_angle, count_to_kick_); //pjh
...
//    accum_ball_position_ = NotFound;
  ...
  // printf("distance_to_ball : %f \n", distance_to_ball);
  // printf("distance_to_walk : %f \n", distance_to_walk);
...
void BallFollower::waitFollowing()
{
  ...
  if (count_not_found_ > NOT_FOUND_THRESHOLD * 0.5)
    setWalkingParam(-0.010, 0.0, 10.0 * M_PI / 180, true);
